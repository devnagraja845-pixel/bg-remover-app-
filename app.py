...
# --- 1. UI SETUP & REFRESH LOCK ---
st.set_page_config(page_title="Ravi Pro Studio AI", layout="centered")
...
# --- HEADER ---
st.title("✨ Ravi Pro Studio AI V6")
...
# --- 2. 20 PROFESSIONAL LUTS LOGIC (Same as before) ---
...
if uploaded_files:
    ...
    st.warning("HD Cutout processing ho rahi hai... Isme thoda time lag sakta hai.")
    ...
    for index, file in enumerate(uploaded_files):
        with st.expander(f"📷 Photo {index+1}: {file.name}", expanded=True):
            ...
            with st.spinner(f"Processing HD: {file.name}..."):
                try:
                    # 🌟 Magic setting to fix edges but keep subject parts 🌟
                    # Foreground/Background thresholds are fine-tuned to not over-cut.
                    processed = remove(
                        img, 
                        alpha_matting=True,
                    ...
                    st.download_button(label=f"💾 Save Ravi_Edit_{index+1}.png", data=buf.getvalue(), file_name=f"Ravi_HD_{file.name.split('.')[0]}.png", mime="image/png", key=f"btn_{index}")
